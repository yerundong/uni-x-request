...
一个基于 `uni.request` 的 uni-app 请求封装库，提供接口服务树、请求缓存、并发去重、Mock、环境重写、Loading/弹窗提示、错误优先级、请求中断、生命周期钩子等开箱即用的能力。
...
> - [九、错误处理机制](#九错误处理机制)
> - [十、Mock 机制](#十mock-机制)
> - [十一、环境重写 rewrite](#十一环境重写-rewrite)
> - [十二、请求打印](#十二请求打印)
> - [十三、响应与返回值](#十三响应与返回值)
...
- **请求中断**：全局 / 实例 / 单请求三档中断，mock 等待、缓存等待等无原生任务的场景也能中断
- **生命周期钩子**：`beforeRequest`、`requestSuccess`、`requestFail`
...
| `webProxy` | `boolean` | `true` | H5 dev 环境是否走本地代理 |
...
```js
{
  ...
  // 其余可配：url/baseUrl/header/timeout/data/params/restful/loading/confirm/successTip/errorTip/cache/print/original
}
```
...
调用 `$api.xxx(options)` 时传入的 options，与"实例默认值 → 接口配置项"依次合并。支持字段如下：
...
请求前弹出确认框（在 `beforeRequest` 钩子**之前**执行），确认才发请求，取消则以 `cancelRequest` 错误终止（不弹任何提示）：
...
**机制**：失败时库会把"预置错误 / 钩子 setErrorMessage / errorTip.message"按优先级合并，取最高者展示，并写入响应 `errCode` / `errMsg`。`enable: false` 的预置错误（如 cancel/abort）不会弹窗。
...
| `webProxy` | `boolean` | `true` | H5 dev 环境走本地代理 |
...
```
合并配置 → confirm → beforeRequest → registerPendingRequest → handleUrl
→ checkNetwork → mock → cache → loading → uni.request
→（2xx）requestSuccess /（其他）requestFail
→ returnRequest（缓存写回 / 提示 / 打印 / resolve·reject）
```

### 6.1 beforeRequest（请求前）
...
### 6.3 requestFail（请求失败）

HTTP 非 2xx 或网络错误时调用，用于补充错误信息：
...
钩子统一接收 `ctx` 对象（也可解构使用），**直接修改其字段即可，返回值会被忽略**：

| 字段 | 说明 |
| --- | --- |
| `request` | 合并后的请求配置，修改后影响后续流程 |
| `response` | 当前响应（修改 `status` 可控制成败） |
| `setErrorMessage({ code, message, priority, enable })` | 设置错误信息，`priority` 默认 1，越高越优先 |
...
UniXRequest.getCacheById('myCacheId')       // 查单个（内存 miss 时查本地存储）
...
| 场景 | 说明 |
| --- | --- |
| 请求发出前被中断 | 直接按中断处理，**不再发出网络请求** |
| 请求在途被中断 | 原生 `requestTask.abort()`，fail 回调 `request:fail abort` |
| Mock 慢响应等待中 | 等待循环感知中断标记，提前退出 |
| 缓存等待中 | 等待者感知中断标记 |

### 8.3 中断后的结果

中断请求一律转为失败：`response.status = 0`、错误码 `001`（`abortRequest` 预置，`enable: false` **不弹任何提示**）、响应 data 被清空，调用方 catch 收到干净的中断错误。
...
cancelRequest:{ code: '000', message: '取消请求', enable: false }     // priority 固定为 Infinity
abortRequest: { code: '001', message: '中止请求', enable: false }     // priority 固定为 Infinity
...
- 中断/取消（`cancelRequest`/`abortRequest`）：`Infinity`，永远胜出（但 `enable: false` 不弹窗）

> `Infinity` 用于中断/取消这类"用户主动终止"的场景：结果确定且不弹提示，避免取消后又弹出业务错误。
...
**网络层失败**（超时、断网、abort 等）不走 HTTP 状态码预置：catch 直接取原生错误的 `errno`/`error` 写入 `errCode`、`errMsg`/`errorMessage` 写入 `errMsg`，再由 `requestFail` 钩子补充处理（如设置业务文案、跳登录）。
...
- `url` 支持字符串或数组，按**正则**匹配请求 url；`method` 需一致
- 配置项里的非控制字段会进入响应体
...
const uxr = new UniXRequest({
  ...
  // 请求前：注入 token
  beforeRequest: async ({ request }) => {
    request.header = { ...request.header, authorization: `Bearer ${getToken()}` }
  },
...
```
src/uni-x-request/
├── index.js        # 主类 UniXRequest：请求主流程、钩子、getApiService
...
失败 reject 的是 `response.data`。要看错误码/文案用 `errCode`/`errMsg`（`original: false` 时可通过 `e` 的字段，或设置 `original: true` 拿完整响应）。

**Q2：请求取消了还会弹"系统异常"？**
不会。`cancelRequest`/`abortRequest` 预置 `enable: false` + `priority: Infinity`，中断请求静默失败，不弹任何提示。
