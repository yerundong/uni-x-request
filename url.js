import { isWeb, isDev, isValidStr, isObj, isStr, isArr } from "./utils";
...
const handleUrl = ({ request, response }, rewriteConfig, baseUrl) => {
  const { url, apiUrl, restful, params, header, devWebProxy } = request;
  ...
  // 网页平台，开发环境，且配置了devWebProxy，则需要请求本地，代理到目标地址
  if (isWeb && isDev && devWebProxy) {
    // 请求地址改为本地地址，代理到目标地址
